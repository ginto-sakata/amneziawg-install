        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            try {
                ...
                for (const [categoryName, category] of Object.entries(data)) {
                    ...
                    for (const [serviceName, service] of Object.entries(category.services)) {
                        ...
                        // Add common TLDs if not present
                        if (!domain.includes('.')) {
                            if (service.url) {
                                domain = service.url;
                            } else if (domain.match(/^[a-z0-9-]+$/)) {
                                domain = domain + '.com';
                            }
                        }
                        ...
                        tooltip.textContent = service.description || `Website for ${serviceName}`;
                        
                        nameContainer.appendChild(name);
                        nameContainer.appendChild(url);
                        
                        serviceDiv.appendChild(checkbox);
                        serviceDiv.appendChild(icon);
                        serviceDiv.appendChild(nameContainer);
                        serviceDiv.appendChild(tooltip);
            ...
            generateBtn.addEventListener('click', function() {
                ...
                if (selectedCIDRs.length > 0) {
                    ...
                    alert('No IP ranges found for selected services');
                }
            });
            
            copyBtn.addEventListener('click', function() {
                if (output.value.trim() === '') {
                    alert('Please generate the IP list first');
                    return;
                }
...
EOL
    ...
    # Copy the favicon downloader script
    cp "${AMNEZIAWG_DIR}/download_favicons.sh" "${WEBSITE_DIR}/download_favicons.sh"
    chmod +x "${WEBSITE_DIR}/download_favicons.sh"
    
    # Generate icons
    echo -e "${GREEN}Downloading service favicons...${NC}"
    "${WEBSITE_DIR}/download_favicons.sh" "${IPLIST_DIR}" "${WEBSITE_DIR}/icons"
    ...
    if command -v python3 &> /dev/null; then
        echo -e "${GREEN}Starting web server using Python 3 at http://${SERVER_IP}:${PORT}${NC}"
